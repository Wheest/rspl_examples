...
# def calculate_partitions(in_h, padding, kdim_h, stride, max_height):
#     # Calculate the total padded height
#     total_padded_height = in_h + 2 * padding

#     # Calculate the height after applying the convolution
#     # Note that the effective height that is influenced by a single row in the partition is
#     # (max_height - kdim_h + stride), not just max_height, due to kernel overlap
#     effective_max_height = max_height - kdim_h + stride

#     # We need at least one kernel height to perform a convolution
#     if effective_max_height < 1:
#         effective_max_height = 1

#     # Calculate partitions, ensuring that we cover the whole height
#     print(
#         total_padded_height,
#         effective_max_height,
#         total_padded_height / effective_max_height,
#     )
#     partitions = math.ceil(total_padded_height / effective_max_height)
...
def generate_large_depth_conv2d(inputs, weights, stride, padding, force_partition=None):
    ...
    #     max_height,
    # )
    # max_height = (in_h + 2 * padding) // h_partitions
    # remainder = (in_h + 2 * padding) % h_partitions
    # if remainder != 0:
    #     print(
    #         f"We have a remainder of {remainder}, so we will have extra 0 calculations"
    #     )

    new_oh = (min(in_h, max_height) - kdim_h) // stride + 1
